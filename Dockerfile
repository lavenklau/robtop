...
RUN apt-get update && apt-get install git cmake libeigen3-dev libgl1-mesa-dev libglu1-mesa-dev libxi-dev libgflags-dev wget libboost-iostreas-dev libtbb-dev libblosc-dev libcgal-dev libglm-dev rapidjson-dev

# download source
RUN cd /home && mkdir src && cd src && git clone https://github.com/lavenklau/robtop.git && cd ./robtop
