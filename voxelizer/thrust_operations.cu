...
float* uploadTriangles(const std::vector<float>& vertex_coords, const std::vector<int>& triface_ids) {	
	...
	for (int i = 0; i < triface_ids.size(); i += 3) {
		int vid = triface_ids[i];
		glm::vec3 v0(vertex_coords[vid * 3 + 0], vertex_coords[vid * 3 + 1], vertex_coords[vid * 3 + 2]);
		vid = triface_ids[i + 1];
		glm::vec3 v1(vertex_coords[vid * 3 + 0], vertex_coords[vid * 3 + 1], vertex_coords[vid * 3 + 2]);
		vid = triface_ids[i + 2];
		glm::vec3 v2(vertex_coords[vid * 3 + 0], vertex_coords[vid * 3 + 1], vertex_coords[vid * 3 + 2]);
		trianglethrust_host->push_back(v0);
		trianglethrust_host->push_back(v1);
		trianglethrust_host->push_back(v2);
	}
